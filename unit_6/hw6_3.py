...
def boggle_words(board, minlength=3):
    ...
    def legal_prefix(prefix):
        return prefix in PREFIXES

    for i, sq in enumerate(board):
        results = results.union(search_words(board, i, legal_prefix, legal_word))
    return results


def search_words(board, start, legal_prefix, legal_word):

    def search_words_internal(loc, prefix, cache):
        if legal_word(prefix):
            results.add(prefix)
        if legal_prefix(prefix):
            for j in neighbors(loc, size(board)):
                if j not in cache:
                    cache.add(j)
                    search_words_internal(j, prefix + board[j], cache)
                    cache.remove(j)

    results = set()
    prefix = board[start]
    search_words_internal(start, prefix, set([start]))
    return results
