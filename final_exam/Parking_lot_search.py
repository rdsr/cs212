...
def path(tree, key):
    result = []
    while tree[key]:
        result.insert(0, tuple(key[1]))  # convert back to tuple
        result.insert(0, key[0])
        key = tree[key]
    result.insert(0, tuple(key[1])) # just add the start state
    return result

def find(key, state):
    k, loc = filter(lambda (k, locs) : k == key, state)[0]
    return loc
...
def goal(state):
    return exit_loc(state)[0] in car_loc(state)
...
def action(new_loc, old_loc):
    return (new_loc[0], max(new_loc[1]) - max(old_loc[1]))

def horizontal(loc):
    return abs(loc[1][0] - loc[1][1]) == 1

def incr_loc(c_loc, i):
    return c_loc[0], tuple(map(lambda p : p + i, c_loc[1]))

def valid_loc(loc, car_loc, board):
    points = set(loc[1]) - set(car_loc[1])
    for i in points:
        # hitting a wall or a car
        if board[i] == '|' or (board[i] != '.' and board[i] != '@'):
            return False
    return True

def valid_locs(car_loc, step, board):
    locs = []
    i = step
    loc = incr_loc(car_loc, i)

    while valid_loc(loc, car_loc, board):
        locs.append(loc)
        i += step
        loc = incr_loc(car_loc, i)
...
def reachable_locs(car_loc, board, N=N):
    "Return all reachable locations for a car"
    step = 1 if horizontal(car_loc) else N
    locs = []

    locs.extend(valid_locs(car_loc, step, board))
    locs.extend(valid_locs(car_loc, -step, board))

    return locs

def neighbors(state, N=N):
    board = make_board(state, N)
    nghbrs = []

    for i, loc in enumerate(state):
        if loc[0] != '@' and loc[0] != '|':  # exit and walls can't change locations
            locs = reachable_locs(loc, board, N)
            new_state = state[:i] + state[i+1:]
            actions_and_states = map(lambda new_loc : (action(new_loc, loc), new_state + (new_loc,)), locs)
            nghbrs.extend(actions_and_states)
...
def solve_parking_puzzle(start, N=N):
    ...
    while queue:
        action, state = queue.pop()
        parent_node = frozenset(state)
        if goal(state):
            return path(tree, (action, parent_node))

        for (n_action, n_state) in neighbors(state, N):
            node = frozenset(n_state)
            if node not in cache:
                tree[(n_action, node)] = action, parent_node
                cache.add(node)
                queue.insert(0, (n_action, n_state))
...
def locs(start, n, incr=1):
    ...
    return tuple(map(lambda i : start + i * incr, range(0, n)))
...
def grid(cars, N=N):
    ...
    def remove_dup(walls):
        return tuple(sorted(set(walls)))

    top_wall    = locs(0, N)
    left_wall   = locs(0, N, N)
    right_wall  = locs(N-1, N, N)
    bottom_wall = locs(N*N - 1, N, -1)

    goal_index  = (N - 1) / 2
    goal        = ('@', (right_wall[goal_index],))
    right_wall  = right_wall[:goal_index] + right_wall[goal_index + 1:]
    walls       = ('|', remove_dup(top_wall + left_wall + right_wall + bottom_wall))

    return (goal,) + cars + (walls,)
...
def show_transitions(transitions):
    ...
    for state in transitions[::2]:
        show(state)
        print

    print "Actions: "
    for action in transitions[1::2]:
        print action

def test():

    assert car_loc(puzzle1) == locs(26, 2)
    assert exit_loc(puzzle1) == locs(31, 1)

    goal_state = puzzle1[:1] + (('*', locs(30, 2)),) + puzzle1[2:]
    assert goal(goal_state)

    assert action(('B', (36, 44, 52)), ('B', (20, 28, 36))) == ('B', 16)
    assert action(('A', (42, 43)), ('A', (45, 46))) == ('A', -3)

    assert incr_loc(('A', (41, 42)), 0) == ('A', (41, 42))
    assert incr_loc(('A', (41, 42)), 1) == ('A', (42, 43))
    assert incr_loc(('B', (12, 20)), N) == ('B', (20, 28))

    assert set(reachable_locs(('B', (20, 28, 36)), make_board(puzzle1), N)) == set([('B', (28, 36, 44)), ('B', (36, 44, 52)), ('B', (12, 20, 28))])
    assert set(reachable_locs(('A', (45, 46)), make_board(puzzle1), N)) == set([('A', (44, 45)), ('A', (43, 44)), ('A', (42, 43))])

    show_transitions(solve_parking_puzzle(puzzle1))
    show_transitions(solve_parking_puzzle(puzzle2))
    show_transitions(solve_parking_puzzle(puzzle3))
    return 'tests pass'
