...
singles = range(20, 0, -1)
doubles = map(lambda x : 2 * x, singles)
triples = map(lambda x : 3 * x, singles)

singles.insert(0, 25)                   # SB
doubles.insert(0, 50)                   # DB

"All possible points that can be acheived in a single shot"
possible_points = sorted(set(triples + doubles  + singles), reverse=True)

def make_point_to_names():
    def update(i, name):
        if i in point_to_names:
            point_to_names[i].append(name)
        else:
            point_to_names[i] = [name]

    point_to_names = {}
    for point in possible_points:
        if point % 3 == 0:
            update(point, 'T'+str(point / 3))
        if point % 2 == 0 and point / 2 <= 20:
            update(point, 'D'+str(point / 2))
        if point <= 20:
            update(point, 'S'+str(point))

    update(50, 'DB')
    update(25, 'SB')

    return point_to_names

point_to_names = make_point_to_names()

def make_name_to_points():
    names = {}
    for point in range(20, 0, -1):
        names['S' + str(point)] = 1 * point
        names['D' + str(point)] = 2 * point
        names['T' + str(point)] = 3 * point
...
name_to_points = make_name_to_points()
...
def double_out(total):
    ...
    def double_out_i(total, di):
        for dart in doubles:
            if total == dart:
                return [random.choice(point_to_names[total])]

        if di == 3:
            return None

        else:
            for dart in possible_points:
                if total > dart:
                    r = double_out_i(total - dart, di + 1)
                    if r : return [random.choice(point_to_names[dart])] + r
            return None

    return double_out_i(total, 1)
...
def ring_outcome(target, miss):
    ...
    # Bull's eye
    if target is 'SB':
        return {'SB': 1 - miss, 'DB': 1/4.0 * miss, 'S': 3/4.0 * miss}
    elif target is 'DB':
        miss += 1/3.0 * (1 - miss)
        return {'DB': 1 - miss, 'SB': 2/3.0 * miss, 'S': 1/3.0 * miss}

    if ring is 'S':
        miss = 1/5.0 * miss
        return {'S': 1 - miss, 'D': miss/2.0, 'T': miss/2.0}
    elif ring is 'D':
        return {'S': miss/2.0, 'D': 1 - miss, 'OFF': miss/2.0}
    else:
        return {'S': miss, 'T': 1 - miss}
...
def bulls_eye(target):
    return target is 'SB' or target is 'DB'

def section_outcome(target, miss):
    if bulls_eye(target):
        o = {'B': 1 - miss}
        for s in sections:
            o[s] = float(miss)/len(sections)
        return o
    else:
        ring, section = target[0], target[1:]
        ccw_sec = ccw(section)
        cw_sec = cw(section)
        return {section: 1 - miss, ccw_sec: miss/2, cw_sec: miss/2}

def outcome_be(r_o, s_o):
    o = {}
    o['SB'] = r_o['SB'] * s_o['B']
    o['DB'] = r_o['DB'] * s_o['B']

    distribute = r_o['S'] * s_o['B'] / 20.0

    for s_k in s_o.keys():
        if s_k is not 'B':
            o['S'+s_k] = r_o['SB'] * s_o[s_k]  +  r_o['S'] * s_o[s_k]  +  r_o['DB'] * s_o[s_k] + distribute
    return o


def outcome(target, miss):
    "Return a probability distribution of [(target, probability)] pairs."
    r_o = ring_outcome(target, miss)
    s_o = section_outcome(target, miss)

    if bulls_eye(target):
        return outcome_be(r_o, s_o)

    o = {}
    for r, s in itertools.product(r_o.keys(), s_o.keys()):
        product = r_o[r] * s_o[s]
        if product != 0: o[r+s] = product
    return o

def expected_score(target, miss):
    o = outcome(target, miss)
    score = 0
    for t, probability in o.items():
        if 'OFF' not in t:
            score = score + name_to_points[t] * probability
    return score

def best_target(miss):
    return max(name_to_points.keys(), key=lambda target : expected_score(target, miss))
