...
def bowling(balls):
    ...
    ## bowling([int, ...]) -> int
    return score(balls, 1, 0)

def score(balls, frame, bonus):
    s = 0
    if frame == 10:
        if balls[0] == 10:              # strike
            s = 10 + balls[1] + balls[2]
        elif balls[0] + balls[1] == 10: # spare
            s = 10 + balls[2]
        else:                           # open
            s = balls[0] + balls[1]
    else:
        if balls[0] == 10:              # strike
            s = 10 + score(balls[1:], frame + 1, balls[1] + balls[2])
        elif balls[0] + balls[1] == 10: # spare
            s = 10 + score(balls[2:], frame + 1, balls[2])
        else:                           # open
            s = balls[0] + balls[1] + score(balls[2:], frame + 1, 0)
