...
def compile_precondition(formula):
    """Builds the precondition that the first
    digit of a number can't be zero, and returns that as a string"""
    words = re.findall('[A-Z]+', formula)
    first_letters = set([word[0] for word in words])
    return ' and '.join(map(lambda l : l + " != 0", first_letters))
...
def compile_formula(formula, verbose=False):
    ...
    body = compile_precondition(formula) + 'and' + ''.join(tokens)
...
def test():
    assert compile_precondition('A + B == BA') == "A != 0 and B != 0"
    assert compile_precondition('YOU == ME**2') == "Y != 0 and M != 0"
